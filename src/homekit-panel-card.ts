...
class HomeKitCard extends LitElement {
	...
	setConfig(config) {
		...
		if (!config.entities && config.rows && !config.enableColumns) {
			throw new Error(
				"If you use rows and columns you need to set enableColumns: true"
			);
		}
		this.config = config;
		this.useTemperature =
			"useTemperature" in this.config
				? this.config.useTemperature
				: false;
		this.useBrightness =
			"useBrightness" in this.config ? this.config.useBrightness : true;
		this.useRGB = "useRGB" in this.config ? this.config.useRGB : true;
		this.rowTitleColor = this.config.titleColor
			? this.config.titleColor
			: false;
		this.horizontalScroll =
			"horizontalScroll" in this.config ? this.config.fullscreen : false;
		this.enableColumns =
			"enableColumns" in this.config ? this.config.enableColumns : false;
		this.statePositionTop =
			"statePositionTop" in this.config
				? this.config.statePositionTop
				: false;
		this.tileHoldAnimation =
			"tileHoldAnimation" in this.config
				? this.config.tileHoldAnimation
				: false;
		this.rulesColor = this.config.rulesColor
			? this.config.rulesColor
			: false;
	...
	render() {
		return html`
			...
			<div class="container${this.enableColumns ? " rows" : ""}">
					...
					? html`
							<div class="header">
								${this.config.title
									...
									: html``}
								<ul
									style="${this.rulesColor
										? "color:" + this.rulesColor
										: ""}"
								>
				...
				${this.enableColumns
					? this._renderRows()
					: this._renderEntities(this.config.entities)}
			</div>
		`;
	}

	firstUpdated() {
		const myNodelist = this.shadowRoot.querySelectorAll(
			"homekit-button.event"
		);
		...
		this.shadowRoot.querySelectorAll(".card-tile").forEach((customCard) => {
			...
			if (customCard.dataset.style) {
				...
				style =
					":host { height: 100%; } ha-card { background: transparent; color: #000; padding: 0!important; box-shadow: none; } .header { padding: 10px 10px 0 10px; } .header .name, .header .name .ellipsis { font-size: 13px!important; font-weight: 500; color: #000; opacity: 1; } .header icon { color: #f7d959; } .states { padding: 0 10px; } .states .state .state__value { font-size: 13px; } .states .state .state__uom { font-size: 13px; margin-top: 0; line-height: normal; } .header .icon { color: #f7d959; }";
			}
			if (style != "") {
				let itterations = 0;
				...
				return html`
					<div class="row">
						${row.columns.map((column) => {
							if (
								column.collapse &&
								this._hasAllEntitiesHidden(column)
							) {
								return ``;
							}
							return html`
								<div
									class="col${column.tileOnRow
										? " fixed"
										: ""}"
									style="${column.tileOnRow
										? "--tile-on-row:" + column.tileOnRow
										: ""}"
								>
	...
	_hasAllEntitiesHidden(column) {
		return column.entities.every((row) =>
			row.entities.every(
				(entity) =>
					entity.hide &&
					this._getTemplate(
	...
	_renderState(ent, stateObj, offStates, type) {
		if (!ent.hideState) {
			if (
				type == "light" &&
				(stateObj.attributes.brightness || ent.state)
			) {
				if (this.statePositionTop) {
					...
					return html`
						<span
							class=" ${offStates.includes(stateObj.state)
								? "value"
								: "value on"}"
							>${this._renderStateValue(
								ent,
								stateObj,
								type
							)}</span
				...
				(type == "sensor" || type == "binary_sensor") &&
				(stateObj.last_changed || ent.state)
			) {
				if (this.statePositionTop) {
					return this._renderCircleState(ent, stateObj, type);
				} else {
				...
				(type == "switch" || type == "input_boolean") &&
				ent.state
			) {
				if (this.statePositionTop) {
					return this._renderCircleState(ent, stateObj, type);
	...
	_renderCircleState(ent, stateObj, type) {
		return html`
			<svg
				class="circle-state"
				viewbox="0 0 100 100"
				style="${stateObj.attributes.brightness && !ent.state
		...
		if (type == "light") {
			return html`
				${stateObj.attributes.brightness && !ent.state
					? html`
							${Math.round(
								stateObj.attributes.brightness / 2.55
							)}%
					  `
					: html``}
				${ent.state && !ent.statePath
					? html`
							${computeStateDisplay(
								this.hass.localize,
				...
				${ent.state && ent.statePath
					? html`
							${this._getValue(ent.state, ent.statePath)}
					  `
					: html``}
			`;
			...
			return html`
				${stateObj.last_changed && !ent.state
					? html`
							${this._calculateTime(stateObj.last_changed)}
					  `
					: html``}
				${ent.state && !ent.statePath
					? html`
	...
	_renderEntities(entities) {
		return html`
            ...
            <div class="card-title" style="${
				this.rowTitleColor ? "color:" + this.rowTitleColor : ""
			}">${row.title}</div>
                <div class="homekit-card${
					this.horizontalScroll === true ? " scroll" : ""
                    ...
                    ${row.entities.map((ent) => {
						if (!ent.card && !ent.custom) {
							...
							if (ent.color) {
								...
								color = this._getColorForLightEntity(
									stateObj,
									this.useTemperature,
									this.useBrightness,
									this.useRGB
							...
							if (type == "light") {
								...
								if (!ent.slider) {
									return stateObj
										? html`
												<homekit-button
													class="event ${offStates.includes(
														stateObj.state
													)
														? "button"
														: "button on"}${ent.noPadding
														? " no-padding"
														: ""}${ent.wider
														? ent.widerSize
															? " size-" +
												...
												>
													<div
														class="button-inner${this
															.statePositionTop
															? " state-top"
															: ""}"
													>
														<span
															class="${offStates.includes(
																stateObj.state
															)
																? "icon"
																: "icon on"}${ent.spin ===
									...
									return stateObj
										? html`
												<homekit-button
													class="event slider ${offStates.includes(
														stateObj.state
													)
														? "button"
														: "button on"}${ent.noPadding
								...
								return stateObj
									? html`
											${entityCount == 4
												...
												: html``}
											<homekit-button
												class="event button size-2 on${ent.hide &&
												this._getTemplate(
													stateObj,
													ent.hide
												)
								...
								if (stateObj.state == "off") {
									...
									stateObj.attributes.hvac_action == "idle"
								) {
									mode = "idle";
								} else {
									mode =
										stateObj.state in modes
											? stateObj.state
											: "unknown-mode";
								}
								return stateObj
								...
								return html`
									<homekit-button
										class="${ent.entity
											...
											? " animate"
											: ""}${ent.hide &&
										this._getTemplate(undefined, ent.hide)
											? " hide"
											: ""}${ent.conditionalClass
											? " " +
											  this._getTemplate(
									...
									>
										<div
											class="button-inner card-tile"
											data-card="${ent.card}"
											data-options="${JSON.stringify(
												ent.cardOptions
											)}"
											data-style="${ent.cardStyle
												? ent.cardStyle
												: ""}"
										></div>
									</homekit-button>
	...
	_customAction(tapAction, entity, row) {
		...
		switch (tapAction.action) {
			case "popup":
				this._createPopup(
					tapAction.entity || entity.entity,
					entity,
					row
				);
	...
	async _createPopup(entity_id, entity, row) {
		if ((row && row.popup) || entity.popup) {
			...
			if (row.popup) {
				...
				popUpCard = Object.assign({}, entity.popup, {
					entity: entity_id,
				});
			}
			const popUpStyle = {
	...
	_applyBrightnessToColor(color, brightness) {
		...
		if (colorObj.isValid) {
			const validColor = colorObj
				.mix("black", 100 - brightness)
				.toString();
	...
	static get styles() {
		return css`
			...
			.row .col.fixed {
				min-width: calc(
					var(--tile-on-row) * calc(var(--tile-width, 100px) + 29px)
				);
				width: calc(
					var(--tile-on-row) * calc(var(--tile-width, 100px) + 29px)
				);
			...
			.button.size-2 {
				width: calc(
					(var(--tile-width, 100px) * 2) + (10px * 2) + (10px * 1) -
						1px
				);
			}
			...
			.button.size-3 {
				width: calc(
					(var(--tile-width, 100px) * 3) + (10px * 4) + (10px * 2) -
						1px
				);
			}
			...
			.button.size-4 {
				width: calc(
					(var(--tile-width, 100px) * 4) + (10px * 6) + (10px * 3) -
						1px
				);
			}
			...
			.button.size-5 {
				width: calc(
					(var(--tile-width, 100px) * 5) + (10px * 8) + (10px * 4) -
						1px
				);
			}
			...
			.button.size-6 {
				width: calc(
					(var(--tile-width, 100px) * 6) + (10px * 10) + (10px * 5) -
						1px
				);
			}
			...
			.button.height-2 {
				height: calc(
					(var(--tile-height, 100px) * 2) + (10px * 2) + (10px * 1) -
						1px
				);
			}
			...
			.button.height-3 {
				height: calc(
					(var(--tile-height, 100px) * 3) + (10px * 4) + (10px * 2) -
						1px
				);
			}
			...
			.button.height-4 {
				height: calc(
					(var(--tile-height, 100px) * 4) + (10px * 6) + (10px * 3) -
						1px
				);
			}
			...
			.button.height-5 {
				height: calc(
					(var(--tile-height, 100px) * 5) + (10px * 8) + (10px * 4) -
						1px
				);
			}
			...
			.button.height-6 {
				height: calc(
					(var(--tile-height, 100px) * 6) + (10px * 10) + (10px * 5) -
						1px
				);
			}
			...
			.button.size-2.no-padding {
				width: calc(
					(var(--tile-width, 100px) * 2) + (10px * 4) + (10px * 1) -
						1px
				);
			}
			...
			.button.size-3.no-padding {
				width: calc(
					(var(--tile-width, 100px) * 3) + (10px * 6) + (10px * 2) -
						1px
				);
			}
			...
			.button.size-4.no-padding {
				width: calc(
					(var(--tile-width, 100px) * 4) + (10px * 8) + (10px * 3) -
						1px
				);
			}
			...
			.button.size-5.no-padding {
				width: calc(
					(var(--tile-width, 100px) * 5) + (10px * 10) + (10px * 4) -
						1px
				);
			}
			...
			.button.size-6.no-padding {
				width: calc(
					(var(--tile-width, 100px) * 6) + (10px * 12) + (10px * 5) -
						1px
				);
			}
			...
			.button.size-2 input[type="range"] {
				width: calc(
					(var(--slider-width, 120px) * 2) + (10px * 1) - 1px
				);
				right: calc(
					50% -
						(
							(
									(var(--slider-width, 120px) * 2) +
										(1 * 10px) - 1px
								) / 2
						)
				);
			...
			.button.size-3 input[type="range"] {
				width: calc(
					(var(--slider-width, 120px) * 3) + (10px * 2) - 1px
				);
				right: calc(
					50% -
						(
							(
									(var(--slider-width, 120px) * 3) +
										(2 * 10px) - 1px
								) / 2
						)
				);
			...
			.button.size-4 input[type="range"] {
				width: calc(
					(var(--slider-width, 120px) * 4) + (10px * 3) - 1px
				);
				right: calc(
					50% -
						(
							(
									(var(--slider-width, 120px) * 4) +
										(3 * 10px) - 1px
								) / 2
						)
				);
			...
			.button.size-5 input[type="range"] {
				width: calc(
					(var(--slider-width, 120px) * 5) + (10px * 4) - 1px
				);
				right: calc(
					50% -
						(
							(
									(var(--slider-width, 120px) * 5) +
										(4 * 10px) - 1px
								) / 2
						)
				);
			...
			.button.height-2 input[type="range"] {
				...
				top: calc(
					50% - (((var(--slider-height, 120px) * 2) / 2) + (5px * 1))
				);
			}
			...
			.button.height-3 input[type="range"] {
				...
				top: calc(
					50% -
						(
							((var(--slider-height, 120px) * 3) / 2) + (5px * 2) -
								0.5px
						)
				);
			}
			...
			.button.height-4 input[type="range"] {
				...
				top: calc(
					50% - (((var(--slider-height, 120px) * 4) / 2) + (5px * 3))
				);
			}
			...
			.button.height-5 input[type="range"] {
				...
				top: calc(
					50% -
						(
							((var(--slider-height, 120px) * 5) / 2) + (5px * 4) -
								0.5px
						)
				);
			}
			...
			.button.height-6 input[type="range"] {
				...
				top: calc(
					50% - (((var(--slider-height, 120px) * 6) / 2) + (5px * 5))
				);
			}
			...
			.button.height-half input[type="range"] {
				...
				top: calc(
					50% - (((var(--slider-height, 120px) * 0.5) / 2) - 1px)
				);
			}
			...
			homekit-button .state.unavailable {
				color: var(
					--tile-unavailable-state-text-color,
					rgba(255, 0, 0, 1)
				);
			}
			...
			@media only screen and (max-width: 768px) {
				...
				.button.height-half {
					height: calc(
						(var(--tile-height-mobile, 90px) * 0.5) - 13px
					);
				}
				...
				.button.no-padding.height-half {
					height: calc(
						(var(--tile-height-mobile, 90px) * 0.5) - 3px + 10px
					);
				}

				.button.size-2 {
					width: calc(
						(var(--tile-width-mobile, 90px) * 2) + (10px * 2) +
							(10px * 1) - 1px
					);
				}
				...
				.button.size-3 {
					width: calc(
						(var(--tile-width-mobile, 90px) * 3) + (10px * 4) +
							(10px * 2) - 1px
					);
				}
				...
				.button.height-2 {
					height: calc(
						(var(--tile-height-mobile, 90px) * 2) + (10px * 2) +
							(10px * 1) - 1px
					);
				}
				...
				.button.height-3 {
					height: calc(
						(var(--tile-height-mobile, 90px) * 3) + (10px * 4) +
							(10px * 2) - 1px
					);
				}
				...
				.button.height-4 {
					height: calc(
						(var(--tile-height-mobile, 90px) * 4) + (10px * 6) +
							(10px * 3) - 1px
					);
				}
				...
				.button.height-5 {
					height: calc(
						(var(--tile-height-mobile, 90px) * 5) + (10px * 8) +
							(10px * 4) - 1px
					);
				}
				...
				.button.height-6 {
					height: calc(
						(var(--tile-height-mobile, 90px) * 6) + (10px * 10) +
							(10px * 5) - 1px
					);
				}
				...
				.button.size-2.no-padding {
					width: calc(
						(var(--tile-width-mobile, 90px) * 2) + (10px * 4) +
							(10px * 1) - 1px
					);
				}
				...
				.button.size-3.no-padding {
					width: calc(
						(var(--tile-width-mobile, 90px) * 3) + (10px * 6) +
							(10px * 2) - 1px
					);
				}
				...
				.button.height-2.no-padding {
					height: calc(
						(var(--tile-height-mobile, 90px) * 2) + (10px * 4) +
							(10px * 1) - 1px
					);
				}
				...
				.button.height-3.no-padding {
					height: calc(
						(var(--tile-height-mobile, 90px) * 3) + (10px * 6) +
							(10px * 2) - 1px
					);
				}
				...
				.button.height-4.no-padding {
					height: calc(
						(var(--tile-height-mobile, 90px) * 4) + (10px * 8) +
							(10px * 3) - 1px
					);
				}
				...
				.button.height-5.no-padding {
					height: calc(
						(var(--tile-height-mobile, 90px) * 5) + (10px * 10) +
							(10px * 4) - 1px
					);
				}
				...
				.button.height-6.no-padding {
					height: calc(
						(var(--tile-height-mobile, 90px) * 6) + (10px * 12) +
							(10px * 5) - 1px
					);
				}
