      ...
      ${this.config.entities.map(t=>{var e=0;return V`
              <div class="card-title">${t.title}</div>
                  <div class="homekit-card">
                      ${t.entities.map(n=>{if(!n.card&&!n.custom){var a=["off","unavailable"];n.offStates&&(a=n.offStates);const s=this.hass.states[n.entity];var o="#f7d959";3==e&&(e=0),4==e&&(e=2),o=n.color?n.color:this._getColorForLightEntity(s,this.config.useTemperature,this.config.useBrightness);var r=n.entity.split(".")[0];if("light"==r)return e++,s?V`
                                <homekit-card-item>
                                  <homekit-button class="${a.includes(s.state)?"button":"button on"}" @action=${e=>this._handleClick(e,s,n,r,t)}>
                                      <div class="button-inner">
                                        <span class="icon" style="${a.includes(s.state)?"":"color:"+o+";"}">
                                          <ha-icon icon="${n.icon||s.attributes.icon||_(b(s.entity_id),s.state)}" class=" ${n.spin&&"on"===s.state?"spin":""}"/>
                                        </span>
                                        <span class="${a.includes(s.state)?"name":"name on"}">${n.name||s.attributes.friendly_name}</span>
                                        <span class="${a.includes(s.state)?"state":"state on"}">
                                          ${v(this.hass.localize,s,this.hass.language)}
                                          ${s.attributes.brightness&&!n.state?V` <span class=" ${a.includes(s.state)?"value":"value on"}">${Math.round(s.attributes.brightness/2.55)}%</span>`:V``}
                                          ${n.state?V` <span class=" ${a.includes(s.state)?"value":"value on"}">${v(this.hass.localize,this.hass.states[n.state],this.hass.language)}</span>`:V``}
                                        </span>
                                      </div>
                                  </homekit-button>
                                </homekit-card-item>
                                ${3==e&&this.config.breakOnMobile?V`<div class="break"></div>`:V``}
                                `:this._notFound(n);if("sensor"==r||"binary_sensor"==r)return e++,s?V`
                              <homekit-card-item>
                                <homekit-button class="${a.includes(s.state)?"button":"button on"}" @action=${e=>this._handleClick(e,s,n,r,t)}>
                                    <div class="button-inner">
                                      <span class="${a.includes(s.state)?"icon":"icon on"}">
                                        <ha-icon icon="${n.icon||s.attributes.icon||_(b(s.entity_id),s.state)}" />
                                      </span>
                                      <span class="${a.includes(s.state)?"name":"name on"}">${n.name||s.attributes.friendly_name}</span>
                                      <span class="${a.includes(s.state)?"state":"state on"}">
                                        ${v(this.hass.localize,s,this.hass.language)}
                              ...
                              ${3==e&&this.config.breakOnMobile?V`<div class="break"></div>`:V``}
                            `:this._notFound(n);if("switch"==r||"input_boolean"==r)return e++,s?V`
                              <homekit-card-item>
                                <homekit-button class="${a.includes(s.state)?"button":"button on"}" @action=${e=>this._handleClick(e,s,n,r,t)}>
                                    <div class="button-inner">
                                      <span class="${a.includes(s.state)?"icon":"icon on"}">
                                        <ha-icon icon="${n.icon||s.attributes.icon}" />
                                      </span>
                                      <span class="${a.includes(s.state)?"name":"name on"}">${n.name||s.attributes.friendly_name}</span>
                                      <span class="${a.includes(s.state)?"state":"state on"}">
                                        ${v(this.hass.localize,s,this.hass.language)}
                                        ${n.state?V` <span class="value on">${v(this.hass.localize,this.hass.states[n.state],this.hass.language)}</span>`:V``}
                                      </span>
                                    </div>
                                </homekit-button>
                              </<homekit-card-item>
                              ${3==e&&this.config.breakOnMobile?V`<div class="break"></div>`:V``}
                            `:this._notFound(n);if("weather"==r)return e+=2,s?V`
                              ${4==e&&this.config.breakOnMobile?V`<div class="break"></div>`:V``}
                              <homekit-card-item>
                                <homekit-button class="button size-2 on" @action=${e=>this._handleClick(e,s,n,r,t)}>
                                    <div class="button-inner">
                                      <span class="icon on">
