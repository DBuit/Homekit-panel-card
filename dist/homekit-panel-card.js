      ...
      ${this.config.entities.map(e=>{var n=0;return V`
              <div class="card-title" style="${t?"color:"+t:""}">${e.title}</div>
                  <div class="homekit-card">
                      ${e.entities.map(t=>{if(!t.card&&!t.custom){var a=["off","unavailable"];t.offStates&&(a=t.offStates);const s=this.hass.states[t.entity];var o="#f7d959";3==n&&(n=0),4==n&&(n=2),o=t.color?t.color:this._getColorForLightEntity(s,this.config.useTemperature,this.config.useBrightness);var i=t.entity.split(".")[0];if("light"==i)return n++,s?V`
                                <homekit-card-item>
                                  <homekit-button class="${a.includes(s.state)?"button":"button on"}" @action=${n=>this._handleClick(n,s,t,i,e)}>
                                      <div class="button-inner">
                                        <span class="icon" style="${a.includes(s.state)?"":"color:"+o+";"}">
                                          <ha-icon icon="${t.offIcon?a.includes(s.state)?t.offIcon:t.icon:t.icon||s.attributes.icon||_(b(s.entity_id),s.state)}" class=" ${t.spin&&"on"===s.state?"spin":""}"/>
                                        </span>
                                ...
                                `:this._notFound(t);if("sensor"==i||"binary_sensor"==i)return n++,s?V`
                              <homekit-card-item>
                                <homekit-button class="${a.includes(s.state)?"button":"button on"}" @action=${n=>this._handleClick(n,s,t,i,e)}>
                                    <div class="button-inner">
                                      <span class="${a.includes(s.state)?"icon":"icon on"}">
                                        <ha-icon icon="${t.offIcon?a.includes(s.state)?t.offIcon:t.icon:t.icon||s.attributes.icon||_(b(s.entity_id),s.state)}" />
                                      </span>
                                      <span class="${a.includes(s.state)?"name":"name on"}">${t.name||s.attributes.friendly_name}</span>
                                      <span class="${a.includes(s.state)?"state":"state on"}">
                                        ${v(this.hass.localize,s,this.hass.language)}
                              ...
                              ${3==n&&this.config.breakOnMobile?V`<div class="break"></div>`:V``}
                            `:this._notFound(t);if("switch"==i||"input_boolean"==i)return n++,s?V`
                              <homekit-card-item>
                                <homekit-button class="${a.includes(s.state)?"button":"button on"}" @action=${n=>this._handleClick(n,s,t,i,e)}>
                                    <div class="button-inner">
                                      <span class="${a.includes(s.state)?"icon":"icon on"}">
                              ...
                              ${3==n&&this.config.breakOnMobile?V`<div class="break"></div>`:V``}
                            `:this._notFound(t);if("weather"==i)return n+=2,s?V`
                              ${4==n&&this.config.breakOnMobile?V`<div class="break"></div>`:V``}
                              <homekit-card-item>
                                <homekit-button class="button size-2 on" @action=${n=>this._handleClick(n,s,t,i,e)}>
                                    <div class="button-inner">
                                      <span class="icon on">
